...
Qwen2.5 thể hiện sự cân bằng đáng chú ý giữa các chỉ số. BLEU đạt 9.19 (vượt qua Lead-3 với 8.79) và BERTScore đạt 70.34, cho thấy mô hình này sinh ra văn bản có độ trùng khớp n-gram liên tiếp và độ tương đồng ngữ nghĩa cao hơn so với BARTpho. Sự vượt trội ở BLEU (+6.94 so với BARTpho) có thể được giải thích bởi sự kết hợp của nhiều yếu tố. Thứ nhất, Qwen2.5 được tiền huấn luyện trên 18 nghìn tỷ tokens đa ngôn ngữ, lớn hơn nhiều bậc so với 20GB dữ liệu tiếng Việt của BARTpho (xem Bảng \ref{tab:model_arch}), cung cấp kiến thức ngôn ngữ phong phú hơn đáng kể. Thứ hai, bộ từ vựng BPE lớn hơn ($\sim$152k tokens so với $\sim$40k syllable) cho phép Qwen2.5 biểu diễn các cụm từ phổ biến bằng ít token hơn, tăng xác suất khớp n-gram liên tiếp với nhãn chuẩn. Thứ ba, kiến trúc Decoder-only sinh mỗi token dựa trên toàn bộ ngữ cảnh trước đó trong cùng một không gian biểu diễn, thay vì phụ thuộc vào cơ chế Cross-Attention gián tiếp giữa Encoder và Decoder. Cần lưu ý rằng sự khác biệt giữa hai mô hình về quy mô dữ liệu tiền huấn luyện, bộ tokenizer, và effective batch size (16 so với 8) khiến việc quy kết toàn bộ chênh lệch hiệu năng cho kiến trúc là chưa hoàn toàn chặt chẽ. ROUGE-1 thấp hơn BARTpho (47.85 so với 50.24) là kết quả phù hợp với mức độ trừu tượng hóa cao hơn: mô hình paraphrase thay vì sao chép trực tiếp từ văn bản gốc, dẫn đến trùng lặp unigram thấp hơn.
...
\midrule
FFT -- BARTpho & \textbf{2.80} & \textbf{3.55} & \textbf{4.06} & \textbf{2.73} \\
LoRA -- BARTpho & 2.73 & 3.48 & 3.89 & 2.67 \\
LoRA -- Qwen2.5 & 2.44 & 2.22 & 3.39 & \textbf{2.73} \\
\bottomrule
\end{tabular}
\end{table}
  ...
  \begin{figure}[H]
      ...
      \caption{Biểu đồ so sánh điểm số LLM-as-a-Judge trên 4 tiêu chí. Qwen2.5 đạt độ trôi chảy (Fluency) tương đương BARTpho FFT nhưng thấp hơn đáng kể ở các tiêu chí cấu trúc và ngữ nghĩa.}
      \label{fig:llm_judge}
  \end{figure}

Về độ trôi chảy (Fluency), Qwen2.5 đạt \textbf{2.73}, ngang bằng với mô hình FFT-BARTpho tốt nhất và cao hơn LoRA-BARTpho (2.67). Phát hiện này phần nào nhất quán với quan sát từ chỉ số BLEU và BERTScore trước đó, gợi ý rằng Qwen2.5 có khả năng sinh văn bản tương đối trôi chảy -- dù cần lưu ý rằng kết quả này cũng có thể chịu ảnh hưởng từ quy mô dữ liệu tiền huấn luyện lớn hơn nhiều bậc của Qwen2.5.

Tuy nhiên, Qwen2.5 lại cho điểm thấp hơn hẳn ở 3 tiêu chí còn lại: Relevance (2.44), Coherence (2.22), và Consistency (3.39). Hiện tượng này xuất phát từ bản chất của checkpoint \texttt{Qwen/Qwen2.5-0.5B}: đây là một mô hình gốc (Base model) chưa trải qua tinh chỉnh chỉ thị (Instruction Tuning) để làm theo cấu trúc tóm tắt chuyên biệt. Do đó, dù văn bản sinh ra trôi chảy, mô hình gặp khó khăn trong việc tổ chức một đoạn tóm tắt có cấu trúc chặt chẽ (Coherence) và chắt lọc đúng trọng tâm (Relevance/Consistency). Ngược lại, BARTpho với chiến lược tiền huấn luyện Denoising Autoencoder được thiết kế chuyên biệt cho các bài toán seq2seq \cite{bart_original}, giúp mô hình bám sát cấu trúc của đoạn tóm tắt tốt hơn.

Cần lưu ý rằng phần lớn các mô hình đều đạt điểm trung bình dưới 4.0/5.0, trong đó tiêu chí quan trọng nhất là Relevance chỉ dao động ở mức 2.44--2.80. Điều này cho thấy chất lượng tóm tắt tự động trên tiếng Việt nhìn chung vẫn còn khoảng cách đáng kể so với kỳ vọng, và việc cải thiện khả năng chắt lọc thông tin trọng tâm (Relevance) cùng tổ chức văn bản mạch lạc (Coherence) là hướng nghiên cứu cần được ưu tiên.
...
\midrule
RQ1 & Seq2Seq vs. Causal LLM & BARTpho bao phủ từ vựng rộng (ROUGE-1: 50.24), Qwen2.5 đạt BLEU và BERTScore cao hơn (BLEU: 9.19 vs 2.25); sự khác biệt chịu ảnh hưởng từ nhiều yếu tố ngoài kiến trúc \\
...
Đối với RQ1, BARTpho và Qwen2.5 thể hiện thế mạnh ở các khía cạnh bổ trợ cho nhau: BARTpho đạt ROUGE-1 cao nhất (50.24) nhờ cơ chế Bidirectional Attention giúp bao phủ từ vựng rộng, trong khi Qwen2.5 đạt BLEU cao hơn (9.19 so với 2.25) và BERTScore vượt trội (70.34 so với 66.46). Tuy nhiên, cần lưu ý rằng sự khác biệt này chịu ảnh hưởng đồng thời từ nhiều yếu tố ngoài kiến trúc, bao gồm quy mô dữ liệu tiền huấn luyện (18T tokens so với 20GB), bộ tokenizer (BPE so với syllable), và effective batch size (8 so với 16). Sự tồn tại của Lead Bias trong dữ liệu tin tức cũng cần được cân nhắc kỹ khi diễn giải các chỉ số.
