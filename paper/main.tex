...
Entity Guided + GTF-Penalty & \textbf{50.00} & \textbf{14.01} & \textbf{29.74} & \textbf{2.57} & \textbf{70.88} \\
\bottomrule
\end{tabular}
}
\end{table}

Kết quả cho thấy sự kết hợp giữa các cải tiến cấu phần lõi (RoPE, RMSNorm, SwiGLU) và cơ chế dẫn hướng thực thể kết hợp GTF-Penalty đã cải thiện đáng kể hiệu năng của mô hình. So với mô hình Baseline, phiên bản hoàn thiện cuối cùng (Entity Guided + GTF-Penalty) cải thiện 15.63 điểm ROUGE-1 và 5.80 điểm BERTScore-F1. Việc GTF-Penalty can thiệp trực tiếp vào thời điểm suy diễn đã giải quyết lỗi lặp từ vòng lặp vô hạn (exposure bias), giúp ROUGE-1 tăng từ 48.79 lên 50.00 so với khi chưa áp dụng mức phạt. Điều này chứng minh rằng việc kiểm soát chiến lược giải mã mang lại lợi ích bổ trợ quan trọng bên cạnh các cải tiến kiến trúc biểu diễn.
...
\paragraph{Nghịch lý ROUGE và Giới hạn của Custom Transformer.}
Một phát hiện mang tính bước ngoặt của nghiên cứu nằm ở kết quả của mô hình tự thiết kế \textbf{Entity Guided + GTF (Custom)}. Dù đạt điểm ROUGE-1 rất ấn tượng (50.00, tương đương các mô hình SOTA), LLM Judge lại chấm mô hình này ở mức gần bét bảng trên mọi tiêu chí ngữ nghĩa (Consistency = 1.02, Relevance = 1.05). Sự chênh lệch này phơi bày một lỗ hổng chí mạng của các thước đo N-gram (như ROUGE) khi đối chiếu với mô hình huấn luyện từ đầu (train-from-scratch) trên tập dữ liệu nhỏ (10.000 bài báo). 

Cụ thể, mô hình Custom Transformer đã học được cách "chụp giật" từ khóa (ví dụ sinh ra chính xác các cụm \textit{"Quân đội Nga"}, \textit{"Syria"}) giúp điểm ROUGE-1 đo độ trùng lặp từ vựng tăng vọt. Tuy nhiên, do thiếu đi bộ tham số tiền huấn luyện (Pre-training) khổng lồ như BARTpho (20GB) hay Qwen2.5 (18 Nghìn Tỷ tokens), mô hình tự thiết kế hoàn toàn thất bại trong việc ráp nối các từ khóa này thành một câu đúng ngữ pháp, dẫn đến các câu kết thúc đột ngột hoặc sai lệch logic hoàn toàn so với bản gốc. LLM Judge đã phát hiện và trừng phạt đích đáng sự suy thoái ngữ nghĩa này. Phát hiện này là lời cảnh tỉnh về việc lạm dụng độ đo ROUGE: trong kỷ nguyên LLM, độ đo truyền thống rất dễ bị "đánh lừa" bởi các mô hình sinh từ khóa rời rạc, và LLM-as-a-judge là công cụ không thể thiếu để đối chiếu chất lượng tạo sinh thực tế.
