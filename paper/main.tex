...
\subsection{Đánh giá Kiến trúc Transformer Tùy chỉnh (Huấn luyện từ đầu)}

Để trả lời RQ1, chúng tôi đánh giá lộ trình cải tiến kiến trúc Transformer được huấn luyện từ đầu trên tập kiểm thử chung (General Set). Bảng \ref{tab:custom_arch_eval} trình bày hiệu năng của các phiên bản cấu hình khác nhau.

\begin{table}[H]
\centering
\caption{Kết quả đánh giá các cấu hình kiến trúc tùy chỉnh huấn luyện từ đầu trên tập kiểm thử chung. Giá trị in đậm biểu thị kết quả tốt nhất. Ghi chú: Cấu hình \textit{Improved Core} có bao gồm SwiGLU; cấu hình \textit{Extended} có bao gồm HyperSphereNorm và Label Smoothing.}
\label{tab:custom_arch_eval}
\resizebox{\textwidth}{!}{
\begin{tabular}{@{}lcccccc@{}}
\toprule
\textbf{Phương pháp} & \textbf{ROUGE-1} & \textbf{ROUGE-2} & \textbf{ROUGE-L} & \textbf{ROUGE-Lsum} & \textbf{BLEU} & \textbf{BERTScore-F1} \\
\midrule
Baseline Transformer (6L) & 47.32 & 12.23 & 28.14 & 28.14 & 7.57 & 71.08 \\
Improved Core (RoPE + RMSNorm) & 48.18 & 13.42 & 29.13 & 29.13 & 6.34 & 71.41 \\
Extended (11L + Weight Tying) & 49.05 & 13.61 & 29.21 & 29.21 & \textbf{7.82} & 71.38 \\
Entity Guided & 47.15 & 13.63 & 28.74 & 28.74 & 5.03 & 71.03 \\
Entity Guided + GTF-Penalty & \textbf{51.96} & \textbf{16.56} & \textbf{30.48} & \textbf{30.48} & 7.76 & \textbf{72.96} \\
\bottomrule
\end{tabular}
}
\end{table}

Kết quả cho thấy sự kết hợp giữa các cải tiến cấu phần lõi (RoPE, RMSNorm, SwiGLU) và cơ chế dẫn hướng thực thể kết hợp GTF-Penalty đã cải thiện đáng kể hiệu năng của mô hình. So với mô hình Baseline, phiên bản hoàn thiện cuối cùng (Entity Guided + GTF-Penalty) cải thiện 4.64 điểm ROUGE-1 và 1.88 điểm BERTScore-F1. Việc GTF-Penalty can thiệp trực tiếp vào thời điểm suy diễn đã giải quyết lỗi lặp từ vòng lặp vô hạn (exposure bias), giúp ROUGE-1 tăng từ 47.15 lên 51.96 so với khi chưa áp dụng mức phạt. Điều này chứng minh rằng việc kiểm soát chiến lược giải mã mang lại lợi ích tương đương các cải tiến kiến trúc biểu diễn.

\subsection{Đánh giá Tổng thể Mô hình Tiền huấn luyện trên Tập Kiểm thử Chung}
...
\textbf{Phương pháp} & \textbf{ROUGE-1} & \textbf{ROUGE-2} & \textbf{ROUGE-L} & \textbf{ROUGE-Lsum} & \textbf{BLEU} & \textbf{BERTScore-F1} \\
\midrule
Lead-3                     & 45.75 & 21.87 & 28.99 & 28.98 & 8.79 & \textbf{72.69} \\
TextRank                   & 36.88 & 18.99 & 24.94 & 24.93 & 6.37 & 71.52 \\
\midrule
FFT -- BARTpho             & 50.24 & 15.27 & 30.52 & 30.51 & 2.25 & 66.46 \\
LoRA -- BARTpho            & 50.17 & 14.93 & 30.40 & 30.41 & 2.34 & 66.29 \\
LoRA -- Qwen2.5            & 47.85 & 20.42 & 30.22 & 30.25 & 9.19 & 70.34 \\
LoRA -- Qwen2.5 + GTF      & \textbf{51.28} & \textbf{23.05} & \textbf{31.26} & \textbf{31.28} & \textbf{10.29} & 72.52 \\
\bottomrule
\end{tabular}}
\end{table*}
...
\paragraph{Lý do GTF-Penalty mang tính đặc trị cho Causal LM.}
Một khía cạnh phương pháp luận quan trọng là tại sao GTF-Penalty lại mang tính đặc trị cho Qwen2.5 (và Custom Transformer) mà không áp dụng cho BARTpho. Sự khác biệt nằm ở cơ chế Attention: BARTpho (Seq2Seq) sở hữu luồng Cross-Attention tách biệt, giúp Decoder liên tục đối chiếu độc lập với ngữ cảnh tĩnh của bài báo từ Encoder, do đó hiếm khi rơi vào vòng lặp vô hạn (chứng minh qua việc BARTpho gốc vẫn đạt ROUGE-1 > 50). Ngược lại, kiến trúc Decoder-only của Qwen2.5 dùng chung không gian Self-Attention cho cả bài báo đầu vào lẫn văn bản đang sinh. Khi tính chất tự hồi quy (auto-regressive) vô tình lặp lại một từ, mô hình lập tức dồn trọng số chú ý vào chính lỗi lặp đó, gây ra hiện tượng \textit{Exposure Bias} nghiêm trọng. Về mặt kỹ thuật, thuật toán GTF-Penalty yêu cầu quyền truy cập vào mảng token \texttt{[Prompt + Generated]} để chỉ phạt các từ lặp trong phần tóm tắt sinh ra mà không phạt các thực thể (entities) được copy từ bài báo. Thông tin ngữ cảnh chéo này khả dụng ở Causal LM nhưng bị chia cắt ở luồng LogitsProcessor của mô hình Seq2Seq. Do đó, GTF-Penalty chính là một sự can thiệp có chủ đích (targeted intervention) nhằm giải phóng nút thắt lặp từ, cho phép Causal LLM khai thác trọn vẹn tri thức tiền huấn luyện để đánh bại kiến trúc Seq2Seq truyền thống.
...
Tuy nhiên, Qwen2.5 lại cho điểm thấp hơn hẳn ở 3 tiêu chí còn lại: Relevance (2.44), Coherence (2.22), và Consistency (3.39). \textit{Cần lưu ý rằng kết quả chấm điểm này được thực hiện trên cấu hình Qwen2.5 nguyên bản (chưa áp dụng thuật toán GTF-Penalty)}. Do đó, mức điểm rất thấp ở tiêu chí Coherence (2.22) chính là minh chứng độc lập (từ góc nhìn của giám khảo AI) cho hiện tượng \textit{Exposure Bias} (kẹt lặp từ) đã được chúng tôi phân tích ở Mục 5.2. Mặc dù văn bản sinh ra có thể trôi chảy ở vài câu đầu (Fluency giữ mức 2.73), mô hình sau đó rơi vào vòng lặp, phá vỡ hoàn toàn tính liên kết logic của đoạn văn (Coherence) và làm suy giảm nặng nề khả năng truyền tải đúng trọng tâm (Relevance/Consistency). Sự sụt giảm này một lần nữa củng cố tầm quan trọng sống còn của việc phải trang bị thuật toán phạt lặp có chọn lọc (như GTF) khi sử dụng kiến trúc Causal LLM cho bài toán tóm tắt.
...
Nhằm định lượng mức độ suy thoái cấu trúc, chúng tôi tính toán Tỷ lệ lặp Trigram (Trigram Repetition Rate) trên các bản tóm tắt. Kết quả cho thấy BARTpho kiểm soát việc lặp từ rất tốt, với tỷ lệ lặp chỉ ở mức 0.55\% (FFT) và 0.69\% (LoRA). Ngược lại, Qwen2.5 (phiên bản chưa áp dụng GTF) có tỷ lệ lặp lên đến 10.32\%. Nghĩa là cứ 10 cụm 3 từ do Qwen2.5 sinh ra thì có 1 cụm bị lặp lại. Sự suy thoái cục bộ này (local degeneration) không chỉ giải thích vì sao Qwen2.5 nhận điểm đánh giá LLM-as-a-Judge thấp về mặt Tính nhất quán (Consistency) và Mạch lạc (Coherence), mà còn là nguyên nhân trực tiếp kéo điểm ROUGE-1 đi xuống (do các từ bị lặp không đóng góp thêm vào việc khớp từ mới trong văn bản chuẩn). Chính sự bế tắc này là động lực nền tảng để chúng tôi phát triển và tích hợp thành công thuật toán GTF-Penalty (đã phân tích tại Mục 5.2), giúp triệt tiêu hoàn toàn tỷ lệ lặp và đẩy hiệu năng sinh văn bản của Causal LLM lên mức SOTA mới.
...
\midrule
RQ1 & Cải tiến cấu trúc lõi và GTF-Penalty & Việc thay thế cấu trúc (RoPE, RMSNorm, SwiGLU) và phạt lặp từ (GTF-Penalty) giúp tăng vọt ROUGE-1 từ 47.32 lên 51.96 ở mô hình huấn luyện từ đầu \\
